...
// --- ფუნქცია განახლებულია პრიორიტეტიზაციის ლოგიკით ---
export const findAllTranslations = (text: string): TranslationResult[] => {
            ...
            for (const section of subCategory.sections) {
                for (const subSection of section.subSections) {
                    for (const keyword of subSection.keywords) {
                        ...
                        const georgianWord = keyword.name.ka.toLowerCase();
                        const englishWord = keyword.name.en.toLowerCase();
                        
                        const result = {
                            original: keyword.name.ka,
                            translated: keyword.name.en,
                        };

                        // 1. ვამოწმებთ ზუსტ დამთხვევას (ორივე ენაში)
                        if (georgianWord === searchText || englishWord === searchText) {
                            exactMatches.push(result);
                            addedKeywordIds.add(keyword.id);
                        } 
                        // 2. თუ ზუსტი დამთხვევა არაა, ვამოწმებთ ნაწილობრივს
                        else if (georgianWord.includes(searchText) || englishWord.includes(searchText)) {
